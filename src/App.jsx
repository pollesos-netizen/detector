...
const KO_NAME_CONTEXT =
  "(?:성명|이름|신청인|민원인|고객명|담당자|작성자|제출자|수신자|작업자|수정자)";
...
const PATTERNS = [
  ...
  },
  {
    ...
    action: "마스킹",
    regex: new RegExp(
      `(?<![가-힣])${KO_NAME_WITH_TITLE_PATTERN}(?=\\s*${KO_NAME_TITLE})`
      + `|(?<=${KO_NAME_CONTEXT}\\s{0,5}[:：]?\\s{0,5})${KO_STANDALONE_NAME_PATTERN}(?=[^가-힣]|$)`,
      "g"
    ),
    desc: "직책·호칭과 결합된 성명, 또는 성명·담당자 등 컨텍스트 단어 뒤에 오는 한국인 성명",
  },
  {
    id: "vlan",
    label: "VLAN/포트 정보",
